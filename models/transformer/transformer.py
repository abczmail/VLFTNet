...
class Transformer(CaptioningModel):
    ...
    def forward(self, mode, images, seq=None, max_len=None, eos_idx=None, beam_size=None, out_size=1, return_probs=False):
        ...
        '''
        if mode == 'xe':
            grid_embed = self.get_pos_embedding(images)             # Grid Augmented Features
            bs, _, vis_dim = images.size()
            # Grid feature
            grid_enc_output, grid_mask_enc = self.encoder(images, pos=grid_embed)
            ...
            grid_embed = torch.cat([grid_embed, pseudo_region_enc_output], dim=1)
            ...
            dec_output = self.decoder(seq, output, mask, pos=grid_embed)

            # dec_output = self.decoder(seq, grid_enc_output, grid_mask_enc, pos=grid_embed)
    ...
    def step(self, t, prev_output, visual, seq, mode='teacher_forcing', **kwargs):
        ...
        if mode == 'teacher_forcing':
            ...
            if t == 0:
                ...
                grid_enc_output, grid_mask_enc = self.encoder(visual, pos=self.grid_embed)
                ...
                self.grid_embed = torch.cat([self.grid_embed, pseudo_region_enc_output], dim=1)
                # self.enc_output = grid_enc_output
                # self.mask_enc = grid_mask_enc
...
class TransformerEnsemble(CaptioningModel):
    def __init__(self, model: Transformer, weight_files):
        ...
        for i in range(self.n):
            ...
            # self.models[i].load_state_dict(state_dict_i, strict=False)
